...
function ActivityFilterBar({ filters, onChange, onReset, active }) {
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-4">
      <div className="grid grid-cols-2 items-end gap-3 sm:flex sm:items-end sm:gap-4 sm:flex-1">
      ...
      {active ? (
        <button
          type="button"
          onClick={onReset}
          className="inline-flex h-11 w-full shrink-0 items-center justify-center gap-1.5 rounded-xl border border-outline-variant px-4 text-sm font-semibold text-on-surface transition-colors hover:bg-surface-container-low sm:w-auto"
        >
